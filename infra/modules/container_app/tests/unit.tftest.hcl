...
variables {
  ...
  jwt_secret_key_secret_versionless_id = "https://kv-test.vault.azure.net/secrets/jwt-secret-key"
...
run "jwt_secret_is_wired" {
  command = plan

  assert {
    condition     = length([for s in azurerm_container_app.app.secret : s if s.name == "jwt-secret-key"]) == 1
    error_message = "Container App must include a JWT Key Vault-backed secret named 'jwt-secret-key'."
  }
}

run "jwt_secret_env_uses_secret" {
  command = plan

  assert {
    condition     = length([for e in azurerm_container_app.app.template[0].container[0].env : e if e.name == "JWT_SECRET_KEY" && e.secret_name == "jwt-secret-key"]) == 1
    error_message = "JWT_SECRET_KEY environment variable must reference the jwt-secret-key secret."
  }
}
